...
// Deferred, debounced contract-aggregate writer (Change I).
//
// For independent (non-sum-to-one) multi markets we take the contract-row write
// out of the per-bet SERIALIZABLE transaction so concurrent answer-bets touch
// only disjoint rows. The contract row holds *eventually-consistent, recomputable*
// display state (volume, uniqueBettorCount, lastBetTime, and the denormalized
// data.answers cache whose source of truth is the `answers` table), so we apply
// it here with atomic SQL — no cross-answer conflict, no retry storm.
//
// Drift on crash is bounded by the debounce window and self-heals via the
// existing backfills (backfill-unique-bettors, backfill-empty-unique-bettor-count).
